...
# Print presence-only for secrets so tee'd command logs and screen-shares don't capture raw values.
echo "DB studio password:     $([ -n "$DB_STUDIO_PASSWORD" ] && echo '<set>' || echo '<not set>')"
echo "Rasa Pro license:       $([ -n "$RASA_PRO_LICENSE" ] && echo '<set>' || echo '<not set>')"
